...
//Arrow Methods - map, filter, reduce

//map

const x = [1, 2, 3, 4, 5].map(el=>el *3)// all elements multiplied by 3
console.log(x);


const someBooks = getBooks();
const titles = someBooks.map(el=>el.title);
console.log(titles);

const essentialBooks = someBooks.map(el=>({
    title: el.title,
    author: el.author,
    reviews: getTotalReviewsCount(el),
}));
